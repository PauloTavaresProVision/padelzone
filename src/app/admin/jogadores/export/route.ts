...
export async function GET() {
  ...
  const csv =
    "﻿" +
    ["Nome;Email;Telefone;Cidade;Inscrições", ...rows.map((r) => `${clean(r.name)};${clean(r.email)};${clean(r.phone)};${clean(r.city)};${r.count}`)].join("\n");

  return new Response(csv, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="jogadores-${club.slug}.csv"`,
    },
  });
}
